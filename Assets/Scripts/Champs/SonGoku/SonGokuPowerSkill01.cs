...
public class SonGokuPowerSkill01 : MonoBehaviourPunCallbacks
{
        ...
        if(_ofPlayer == "Player01")
        {
            if (collision.gameObject.CompareTag("Player02"))
            {
                ...
                if(_isOwner)
                {
                    ...
                    PhotonNetwork.Instantiate("SkillSonGoku/ExplosionSkill01", new Vector2(transform.position.x, transform.position.y +  0.3f), Quaternion.identity);                    
                }
                DestroyThisRPC();
            }
        }
