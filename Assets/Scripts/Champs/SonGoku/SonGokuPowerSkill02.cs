...
public class SonGokuPowerSkill02 : MonoBehaviourPunCallbacks
{
        ...
        if(_ofPlayer == "Player01")
        {
            if (collision.gameObject.CompareTag("Player02"))
            {
                ...
                if(_isOwner)
                {
                    ...
                    {
                        PhotonNetwork.Instantiate("SkillSonGoku/ExplosionSkill02", new Vector2(transform.position.x + 0.5f, transform.position.y +  1.4f), Quaternion.identity);
                    }
                    else
                    {
                        PhotonNetwork.Instantiate("SkillSonGoku/ExplosionSkill02", new Vector2(transform.position.x - 0.5f, transform.position.y +  1.4f), Quaternion.identity);                    
