...
public class LuffySkill03 : MonoBehaviourPunCallbacks
{
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        if(_ofPlayer == "Player01")
        {
            if (collision.gameObject.CompareTag("Player02"))
            {
                ...
                {
                    collision.transform.position = new Vector2(collision.transform.position.x - 0.02f, collision.transform.position.y);
                }
                else
                {
                    collision.transform.position = new Vector2(collision.transform.position.x + 0.02f, collision.transform.position.y);
                ...
                if (_isOwner)
                {
                    float _damage =  PlayerPrefs.GetFloat("DamSkill03");
                    _damage = _damage / 90;
