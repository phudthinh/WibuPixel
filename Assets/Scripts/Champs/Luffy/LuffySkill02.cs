...
public class LuffySkill02 : MonoBehaviourPunCallbacks
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(_ofPlayer == "Player01")
        {
            if (collision.gameObject.CompareTag("Player02"))
            {
                ...
                {
                    collision.transform.position = new Vector2(collision.transform.position.x - 2.0f, collision.transform.position.y);
                }
                else
                {
                    collision.transform.position = new Vector2(collision.transform.position.x + 2.0f, collision.transform.position.y);
                ...
                if (_isOwner)
                {
                    float _damage =  PlayerPrefs.GetFloat("DamSkill02");
