...
public class LuffyIntrinsic : MonoBehaviourPunCallbacks
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(_ofPlayer == "Player01")
        {
            if (collision.gameObject.CompareTag("Player02"))
            {
                ...
                if (_isOwner)
                {
                    float _damage = _game.GetDamagePlayer01() + PlayerPrefs.GetFloat("DamIntrinsic");
        ...
        else if(_ofPlayer == "Player02")
        {
            if (collision.gameObject.CompareTag("Player01"))
            {
                ...
                if (_isOwner)
                {
                    float _damage = _game.GetDamagePlayer02()  + PlayerPrefs.GetFloat("DamIntrinsic");
