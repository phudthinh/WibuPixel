...
public class NarutoPowerSkill01 : MonoBehaviourPunCallbacks
{
    ...
    private float _speedFly = 15;
    private int _direction;
    private bool _isOwner;
    private int _numKunai;
    public void SetAttackInfo(string ofPlayerValue, float timeHitboxValue, bool isOwnerValue, int numKunai)
    {
    ...
    void Start()
    {
        ...
        _direction = this.transform.localScale.x > 0 ? 1 : -1;
        ...
        {
            _rb.linearVelocity = new Vector2(_speedFly * _direction, (_speedFly / 7));
            if(_direction == 1)
            {
                this.transform.rotation = Quaternion.Euler(this.transform.rotation.eulerAngles.x, this.transform.rotation.eulerAngles.y, 12f);
            }
            else
            {
                this.transform.rotation = Quaternion.Euler(this.transform.rotation.eulerAngles.x, this.transform.rotation.eulerAngles.y, -12f);
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if(_ofPlayer == "Player01")
        {
            if (collision.gameObject.CompareTag("Player02"))
            {
                ...
                if(_isOwner)
                {
                    float _damage = PlayerPrefs.GetFloat("DamSkill01");
                    ...
                    if(typeOfAttack == "Damage")
                    {
                        PhotonNetwork.Instantiate("EffectHit", new Vector2(randomPositionX, randomPositionY + 0.5f), Quaternion.identity);
                    }
                    else if(typeOfAttack == "CriticalDamage")
                    {
                        PhotonNetwork.Instantiate("EffectCriticalHit", new Vector2(randomPositionX, randomPositionY + 0.5f), Quaternion.identity);
                    }
